...
export default function MoneyTracker() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await getYearMonthData(2021, 1);
      // convert kebab case to camel case
      const parsedData = JSON.parse(
        JSON.stringify(data).replace(/-([a-z])/g, function (g) {
          return g[1].toUpperCase();
        }),
      );

      setIsLoading(false);
      setMoneyIn(parsedData.moneyIn);
      setSinglePayments(parsedData.moneyOut);
      setMultiPayments(parsedData.moneyOutByCategory);
    };
